import { View, Text, Modal, TextInput, TouchableOpacity, StyleSheet, Button, Alert, Pressable,ScrollView  } from 'react-native'
...
const Sayfa3 = () => {
  ...
  const { hesaplaAkademikTesvikPuan,tumKayitlariSilSayfa2,kayitlarSayfa2, toplamPuanSayfa2, kayitEkleSayfa2, kayitSilSayfa2 } = useContext(KayitContext);
 
  const handleKaydet1 = () => kayitEkleSayfa2({oran: 15,ay: ay1,}, () => setModal1Visible(false));
  const handleKaydet2 = () => kayitEkleSayfa2({oran: 10,ay: ay2,}, () => setModal2Visible(false));

  const [modal1Visible, setModal1Visible] = useState(false);
  const [modal2Visible, setModal2Visible] = useState(false);
  const [ay1, setAy1] = useState('');
  const [ay2, setAy2] = useState('');
...
<View style={{ backgroundColor: '#1976d2', padding: 10, borderRadius: 10, marginVertical: 20 }}>
  <Text style={{ fontSize: 20, fontWeight: 'bold', color: 'white' }}>2. Araştırma (15 puan)</Text>
</View>

<View style={styles.satirStil}>
  <Text style={styles.metinStil} numberOfLines={2}>
    2.1. Yurt dışı araştırma	
...
<View style={styles.satirStil}>
  <Text style={styles.metinStil} numberOfLines={2}>2.2. Yurt içi araştırma	
</Text>
      ...
      <Modal transparent visible={modal1Visible} animationType="fade" onRequestClose={() => setModal1Visible(false)}>
        <View style={styles.modalArkaPlan}>
          <View style={styles.modalIcerik}>
            ...
            <Text style={styles.baslik}>Süre (ay)</Text>
            <TextInput
              style={styles.input}
  ...
  <View style={{ marginVertical: 20, width: '90%' }}>
    ...
    {kayitlarSayfa2.length === 0 && <Text>Henüz puan eklenmedi.</Text>}
    {kayitlarSayfa2.map((k, index) => (
    <View key={index} style={{
      ...
      <View>
        ...
        <Text>Ay: {k.ay}</Text>
        <Text>Puan: {k.puan}</Text>
      </View>
<TouchableOpacity onPress={() => kayitSilSayfa2(index)} style={{
...
<View style={{
  ...
  <Text style={{
    ...
    Toplam Puan: {toplamPuanSayfa2.toFixed(2)}
  </Text>
</View>
        ...
        <View style={styles.butonContainer}>
          <TouchableOpacity style={styles.butonTemizle} onPress={tumKayitlariSilSayfa2}>
